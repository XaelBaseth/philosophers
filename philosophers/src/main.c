...
int	initialise_philosophers(general_t *general)
{
	...
	i = 0;
	general->philosophers = malloc(sizeof(philo_t) * general->number_of_philosophers);
	...
	while (i < general->number_of_philosophers)
	{
		...
		general->philosophers[i].right_fork = &general->fork_mutex[(i + 1) % general->number_of_philosophers];
...
int	mutex(general_t *general)
{
	...
	i = -1;
	general->fork_mutex = malloc(sizeof(pthread_mutex_t) * general->number_of_philosophers);
